...
class settings extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Blog.Settings.View');
	...
	*/
	...
	{
		$this->auth->restrict('Blog.Settings.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_blog())
			{
				...
				Template::redirect(SITE_AREA .'/settings/blog');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/settings/blog');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Blog.Settings.Edit');
		...
		{
			$this->auth->restrict('Blog.Settings.Delete');
